...
export default function DetailMonthPage() {
    ...
    const fetchMonthlyRevenue = async (selectedMonth) => {
        ...
        try {
            const response = await api.get(`/chart/revenue/monthly/${selectedMonth}`);
    ...
    const handleFetchMonthRevenue = async () => {
        const formattedMonth = month.replace(/-/g, ''); // yyyyMMdd
        try {
            const response = await api.get(`/chart/revenue/daily/${formattedMonth}`);
            setMonthlyRevenue(response.data.data.totalRevenue);
            setNumUserMonth(response.data.data.revenue.length);
            setDataUser({ users: response.data.data.users, revenue: response.data.data.revenue });
                ...
                <div id="content" className="bg-white/10 col-span-9 rounded-lg p-6">
                    ...
                    <div id="statistic">
                        ...
                        <div className="mb-4">
                            ...
                            />
                            <button onClick={handleFetchMonthRevenue}
                    ...
                    {/* Bảng */}
                    <div className={'mt-10'}>
                        <h1 className="font-bold pb-4 uppercase">Danh sách khách hàng: {month}</h1>
