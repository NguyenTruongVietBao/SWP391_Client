    ...
    useEffect(() => {
        const fetchEnrollmentId = async () => {
            try {
                ...
                if (lessons.length > 0) {
                    setFirstLessonId(lessons[0].lesson_id);
                }
            } catch (error) {
        ...
        <div className="bg-mathcha min-h-screen">
            ...
            <div className="flex flex-row py-12 px-10">
                ...
                <div className="w-3/12 mr-10">
                    <div className="w-full">
                        <div className="mx-auto w-full rounded-xl bg-yellow-100 border-4 border-black">
                            ...
                            <h1 className="text-2xl my-4 font-bold text-center">Chủ đề: {topic.title}</h1>
                            <div className="text-center my-4">
                                <span className="font-medium text-lg">
                                    Thời gian còn lại: <span className={'border-2 border-black rounded-lg bg-blue-200 px-2 py-1'}>{formatTime(timeLeft)}</span>
                                </span>
                            </div>
                        </div>
                        <div className={'flex items-center justify-start my-7'}>
                            {firstLessonId && (
                                <Link
                                    ...
                                    to={`/learning/course/${courseId}/topic/${topicId}/lesson/${firstLessonId}`}>
                ...
                <div className="w-9/12">
                    <div className="bg-no-repeat bg-blue-50 border-4 border-black rounded-xl w-full h-120 py-3 flex flex-col items-center">
                                        ...
                                        {question.option.map((option, i) => (
                                            <li key={i} className="flex items-center">
                                                <input
                                                    ...
                                                    className="mr-2"
                                                />
                                                <label className="text-base">{`${optionLabels[i]}. ${option}`}</label>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
